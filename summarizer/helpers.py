...
STOPWORDS = set([
    ...
    "rappler", "rapplercom", "inquirer", "home",
...
def get_keywords(text):
    """
    This function returns the top 10 keywords and their frequency
    scores. It ignores blacklisted words in STOPWORDS, and
    then counts the number of occurrences of each word
    """
    text = split_words(text)
    num_words = len(text)  # of words before removing blacklist words
    frequency = Counter(x for x in text if x not in STOPWORDS)

    min_size = min(NO_OF_KEYWORDS, len(frequency))  # get first 10
    keywords = {x: y for x, y in frequency.most_common(min_size)}  # recreate a dict
...
def length_score(sentence):
    '''
    How many relevant words are in sentence weighed by number of words
    '''
...
def sbs(words, keywords):
    ...
    for word in words:
        if word in keywords:
            score += keywords[word]
    return (1.0 / fabs(len(words)) * score)/NO_OF_KEYWORDS
